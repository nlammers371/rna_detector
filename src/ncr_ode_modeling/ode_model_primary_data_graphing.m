...
As = repelem(A10, points);%[0, 0.0000002, 0.000002, 0.00002, 0.0002, 0.002, 0.02, 0.2, 2, 20];
...
to_plot = {'C13', 'nC13', 'C13:G1','C13:G1::A1'};
f_indices = ismember(full_reactant_list,to_plot); % get indices of fluorescent reporter
% specify conditions to include in fit
hold on
fluo_cell_raw = cell(1,length(y0_cell));
time_cell_raw = cell(1,length(y0_cell));
...
for f = 1:length(y0_cell)
    y0_cell{f}
    [t_fit,y_fit] = ode15s(@(t,y) ncr_solver_v2(t,y,rate_vec_val,Q),[0 t_max],y0_cell{f});
    p = plot(t_fit,y_fit(:,f_indices));
    legend(full_reactant_list(f_indices),'Location','northwest')
    fluo_cell_raw{f} = sum(y_fit(:,f_indices),2);
    time_cell_raw{f} = t_fit;
end
