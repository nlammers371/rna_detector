...
guide2_row = find(contains(MapCell(:,1),'100nM NCR_034'));
...
for e = 1:length(exp_cell)
  ...
  if e < 3
    ...
    spec_NCR_rows = [guide2_row find(contains(MapCell(:,1),exp_cell{e}))'];
  end
  ...
  for n = 1:numel(ncr_index_vec)
      ...
      if ~isempty(act_ind)
        act_val = MapCell{act_ind+min(activator1_rows)-1,2};
      else
        act_val = 0;
      end

      plot_struct(n).act_ncr_groups = ncr_ind;
      plot_struct(n).act_prim_groups = prim_ind;
     
      % record
      plot_struct(n).activator_rows = activator1_rows;
      plot_struct(n).ac_ind = act_ind;
      plot_struct(n).ac_val = act_val;
      
      % pull raw values to plot
      plot_struct(n).ncr_cols_orig = find(ismember(group_id_vec,plot_struct(n).act_ncr_groups));
      plot_struct(n).ncr_data = cell2mat(ExpCell(2:end,plot_struct(n).ncr_cols_orig));
      plot_struct(n).ncr_data = plot_struct(n).ncr_data - plot_struct(n).ncr_data(1,:);
      
      plot_struct(n).primary_cols_orig = find(ismember(group_id_vec,plot_struct(n).act_prim_groups));
      plot_struct(n).primary_data = cell2mat(ExpCell(2:end,plot_struct(n).primary_cols_orig));
      plot_struct(n).primary_data = plot_struct(n).primary_data - plot_struct(n).primary_data(1,:);
      
      % flag potential outlier series
      plot_struct(n).ncr_outlier_flags = prctile(isoutlier(plot_struct(n).ncr_data,2),33)==1;
      plot_struct(n).primary_outlier_flags = prctile(isoutlier(plot_struct(n).primary_data,2),33)==1;
      
      % calculate summary statistics
      plot_struct(n).ncr_mean = nanmean(plot_struct(n).ncr_data(:,~plot_struct(n).ncr_outlier_flags),2);
      plot_struct(n).ncr_std = nanstd(plot_struct(n).ncr_data(:,~plot_struct(n).ncr_outlier_flags),[],2);
      plot_struct(n).primary_mean = nanmean(plot_struct(n).primary_data(:,~plot_struct(n).primary_outlier_flags),2);
      plot_struct(n).primary_std = nanstd(plot_struct(n).primary_data(:,~plot_struct(n).primary_outlier_flags),[],2);
