...
project_cell = {'05142020_NCRData_37C_100nM NCR_018'};%{'05142020_NCRData_37C_100nM NCR_018','05142020_NCRData_37C_100nM NCR_266','05142020_NCRData_37C_100nM NCR_061'};
x_lim_cell = {[0 1200/60]};
...
%   activator_rows = plot_struct(1).activator_rows;
  ...
  max_delta_array = NaN(length(plot_struct)-1,2);
  t_max_array = NaN(length(plot_struct)-1,2);

  for a = 1:length(plot_struct)-1
    close all
    % get activator val
    ...
    xlim(x_lim)
    % save
    saveas(full_plot,[FigPath 'raw_data_scatter' num2str(1e6*ac_val) 'fM.png'])

%     % mean and se plots
%     mean_plot = figure('Visible','off');
%     hold on  
...
%     xlim(x_lim)
%     % save
%     saveas(mean_plot,[FigPath mean_data_plot.png'])    
    ...
    % save
    saveas(mean_ref_plot,[FigPath 'mean_ref_data_plot_' num2str(1e6*ac_val) 'fM_.png']) 
...
%     end
    ...
    ylim(y_lim)
    % save
    saveas(diff_plot,[FigPath 'diff_data_plot_' num2str(1e6*ac_val) 'fM.png']) 
