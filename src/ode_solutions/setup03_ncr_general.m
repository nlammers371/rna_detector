...
syms kon koff_ns koff_s kcat_ratio kcat_cas sG1 sG2 sT1 sT2 sT2I sS
...
atom_sub_rate_vec = [0 0 sG1 sG2 sT1 sT2 sT2I sS];
...
for i = sub_indices
    atom_str = atom_string_cell{i};
    if ~strcmp(atom_str,'T2I') % exclude cafed activator
        cat_product_string_cell = [cat_product_string_cell {[atom_str 'Z']}];
    end
end
n_products = numel(cat_product_string_cell);
...
colon_factors = [0 0 1 3 6];
iter = 1;
for a1 = 1:n_atoms_total
    for a2 = a1:n_atoms_total % allow for self-interactions                
        % generate compound string
        colon_count = max([count(full_atom_cell{a1},':') count(full_atom_cell{a2},':')]);
        colon_ind = find(colon_factors==colon_count,1,'last');
        cp_string = [full_atom_cell{a1} repelem(':', colon_factors(colon_ind) - colon_factors(colon_ind-1) + 1)...
            full_atom_cell{a2}];        
        ...
        if ~spec_flag            
            ...
            if numel(cat_indices) == 1 && numel(sub_indices) == 1                
                if sub_indices == 1
                    ...
                    sub_str = full_atom_cell{a1};
                    pd_str = [sub_str 'Z'];
                    if strcmp(sub_str,'T2I')
                        pd_str = 'T2';
                    end
                    pd_ind = find(strcmp(full_atom_cell,pd_str));                    
                    cat_cat_ind_vec = [cat_cat_ind_vec a2];
                else
                    cat_rate_vec_sym = [cat_rate_vec_sym full_cat_rate_vec(a1)*full_sub_rate_vec(a2)];
                    sub_str = full_atom_cell{a2};
                    pd_str = [sub_str 'Z'];
                    if strcmp(sub_str,'T2I')
                        pd_str = 'T2';
                    end
...
% array to track all reactants 
full_reactant_cell = full_atom_cell;

% initialize vector objects to store reaction rate info
rate_vec_zero_order_sym = sym('NA',[1,n_reactions]);
...
for i = 1:numel(full_compound_string_cell)  
    ...
    cpnd_ind = find(strcmp(full_reactant_cell,cpnd_str));
    if isempty(cpnd_ind)
        full_reactant_cell = [full_reactant_cell {cpnd_str}];
        cpnd_ind = length(full_reactant_cell);
    end
    ...
    rate_vec_zero_order_sym(rate_ind) = on_rate_vec_sym(i);
    ...
    rate_vec_zero_order_sym(rate_ind) = off_rate;
    ...
    if cat_rate~=0
        ...
        rate_vec_zero_order_sym(rate_ind) = cat_rate;
        ...
        % update special case vec
        catalytic_activity_flag_vec = [catalytic_activity_flag_vec 2];
        specific_activity_flag_vec = [specific_activity_flag_vec logical(off_rate~=koff_ns)];
